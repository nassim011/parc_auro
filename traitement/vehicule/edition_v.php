...
if (isset($_POST['matriculem']) && isset($_POST['datem']) && isset($_POST['wilayam']) && isset($_POST['Marquem']) && isset($_POST['Modelm']) && isset($_POST['Couleurm']) ){
    ...
    if (preg_match('#^[0-9]{2}-[0-9]{3}-[0-9]{1,}$#',$m)){
        ...
        if ($verifwilaya==$wi){
            $oldm=$_SESSION['edition'];
            $pdo=inssertion($pdo,"UPDATE vehicule SET matricule='$m' ,mise_circulation='$date' ,wilaya_m='$wi',marque='$ma',modele='$mo',couleur='$c' WHERE matricule='$oldm'");
            echo "<div class='alert alert-success'>";
            echo "<strong>Success!</strong> Vous Avez Bien Modifier : </div>";
        }
        else{
            echo "<div class='alert alert-danger'>";
            echo "<strong>Error!</strong> Wilaya different du matricule : </div>";
        }
